...
class Checkout_model extends My_Model
{
	...
	public function mani_checkout_total($userid=''){
		...
		$checkoutAmt = 0; $checkoutShippingAmt = 0; $checkoutTaxAmt = 0; $cartDiscountAmt = 0;
...
$cartDiscountAmt = $cartDiscountAmt +  $CartRow->discountAmount;			
		...
		//if($query->row()->discountAmount !=''){
			$grantAmt = $grantAmt - $cartDiscountAmt;
		//}
		
		return number_format($checkoutAmt,2,'.','').'|'.number_format($checkoutSAmt,2,'.','').'|'.number_format($checkoutTAmt,2,'.','').'|'.number_format($grantAmt,2,'.','').'|'.$countVal.'|'.number_format($cartDiscountAmt,2,'.','').'|'.$Shipping_Val->row()->shippingid;
