...
<br>
						<div itemprop="brand" itemscope itemtype="http://schema.org/Brand" itemref="_logo6" style="text-align:left;"><?php if($this->lang->line('user_by') != '') { echo stripslashes($this->lang->line('user_by')); } else echo "by"; ?> <a href="<?php if ($productDetails->row()->user_id != '0'){echo base_url().'user/'.$productDetails->row()->user_name;}else {echo base_url().'user/administrator';}?>" class="username"><span itemprop="name"><?php if ($productDetails->row()->user_id != '0'){echo $productDetails->row()->full_name;}else {echo 'administrator';}?></span></a><div class="rating_star" style="display: inline-block;float:none;">
                        				<?php foreach($product_feedback as $feedbacks) {  $totals = $totals+$feedbacks['rating']; }  $totalratingstars = $totals/count($product_feedback);  ?>
                                        <div class="rat_star1" style="width:<?php echo $totalratingstars * 20; ?>%"></div></div> (<?php  echo $rownum = count($product_feedback); ?>)<span class="icon-heart-filled" style="float: right;padding-right: 10px;color: firebrick;"><?php echo $productDetails->row()->likes; ?> Likes</span></div>
</br>
...
<li>* Avoid extremes in temperature and humidity. Keep away from heaters, dehumidifiers and air conditioners. Dry head will cause wood to dry and shrink, which will cause cracks. </li>
<li>* Use table clothes, runners, coasters and table pads on heavily used coffee and dining tables. </li>
<li>* Hot pads should always be used when placing warm serving dishes on a tabletop. Very hot items such as a sauce pan or baking dish should never be placed on a tabletop, even with a hot pad. </li>
<li>* When enjoying the warm glow from a candle, be sure to place a candle stand or tray underneath the candle to collect any melted wax that may drip or run down the candle. </li>
...
<ul>
<li>* Try to keep your leather furniture positioned away from windows where it will be exposed to the sun, instead use fabric-upholstered furniture in those locations. </li>
...
<li>* Clean spills immediately with dry cloth. Use as little water as possible to clean the spill, and wipe the area dry afterwards. </li>
<li>* Avoid soaking the leather in water or soap. Doing this can damage the leather even more than the initial stain you are trying to remove. </li>
...
<li>* If you accidently spill something on your furniture, wipe excess away (do not rub) with a slightly dampened sponge immediately. </li>
</ul>
<br>
...
<ul>
<li>* Vacuum wicker furniture routinely with the soft brush attachment on yourvacuum cleaner to remove surface debris. </li>
...
<li>* Use tweezers to pick out lint and trapped pet hair that the vacuum and soft brush can’t remove. Wipe the wicker furniture with a soft cloth dipped in a mixture of detergent and water. </li>
</ul>
<br>
...
<img src="/images/Color Guide Sheesham Wood.png" alt="Sheesham Wood Color Guide"  style="width: 590px;height: 300px;">
...
$.ajax({
  ...
  success: function(json){
		if(json.status_code == 1){
				...
				$("#New_Comment").append( '<li><a class="milestone" id="comment-1866615"></a><span class="vcard"><a href="<?php echo base_url();?>user/<?php echo $user_name_ud;?>" class="url"><img src="images/users/<?php if($thumbnail_ud!=''){ echo $thumbnail_ud;}else{echo 'user-thumb.png';}?>" alt="" class="photo"><span class="fn nickname"><?php echo ucfirst($user_name_ud);?></span></a></span><p class="c-text">'+ comments +'</p><p style="float:left;width:100%;text-align:left;font-size: 11px; color: #188A0E;"><?php if($this->lang->line('wait_for_approve') != '') { echo stripslashes($this->lang->line('wait_for_approve')); } else echo "Waiting for approval"; ?><a style="font-size: 11px; color: #f33;margin-left:10px" onclick="javascript:deleteCmt(this);" data-tid="<?php echo $productDetails->row()->seller_product_id;?>" data-cid="'+ json.comment_ID +'"><?php if($this->lang->line('shipping_delete') != '') { echo stripslashes($this->lang->line('shipping_delete')); } else echo "Delete"; ?></a></p></li>' );
...
<div style="padding: 10px 10px 10px 20px" class="fb-like" data-href="https://facebook.com/socktail" data-width="230" data-layout="standard" data-action="like" data-show-faces="true" data-share="false"></div>
</div>
