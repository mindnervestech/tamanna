...
?>
  ...
  <div class="popup sign signup signin-overlay" style="display:none;">
    <div class="popup_wrap update2">
     <div  style="font-size: 24px;  color: #32353c;  padding-bottom: 10px;  text-align: center;padding: 0 0 10px 0;  font-size: 15px;  line-height: 24px;
">    <?php foreach($layoutfulllist->result() as $layoutListRow){ 
      
       if($layoutListRow->place == 'signup title'  ){
	  echo  $layoutListRow->text; ?></div>
	<?php 
			
	  } } ?>
      
      
        <h4 class="stit">    <?php foreach($layoutfulllist->result() as $layoutListRow){ 
      
       if($layoutListRow->place == 'signup description'){
	  echo  $layoutListRow->text; ?></h4>
	<?php 
...
?>          
      ...
      <h4 class="stit" style="cursor: text;"><?php if($this->lang->line('signup_with_emailaddrs') != '') { echo stripslashes($this->lang->line('signup_with_emailaddrs')); } else echo "Sign up with your email address"; ?> </h4>
      <p>
	...
	<div class="popup_wrap">
		<div  style="font-size: 24px;  color: #32353c;  padding-bottom: 10px;  text-align: center;padding: 0 0 10px 0;  font-size: 15px;  line-height: 24px;
"><?php if($this->lang->line('header_almost_done') != '') { echo stripslashes($this->lang->line('header_almost_done')); } else echo "Almost Done!"; ?></div>
		<h4 class="stit"><?php if($this->lang->line('header_more_details') != '') { echo stripslashes($this->lang->line('header_more_details')); } else echo "We need a few more things to set up your account."; ?></h4>
            <fieldset class="frm">
