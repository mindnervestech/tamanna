...
$condition = array('id'=>$user_id,'status'=>'Active');
	...
	public function display_product_detail(){
		...
		if ($this->data['productDetails']->num_rows()==1){
			...
			if (count($catArr)>0){
				...
				$breadCumps .= '<ul class="breadcrumbsNew">
        					    <li style="display:inline-block;"><a class="shop-home" href="'.base_url().'shopby/all">Shop &nbsp;</a></li>';
				foreach ($catArr as $cat){
					if($cat != ''){
		...
		if ($this->data['productDetails']->row()->meta_title != ''){
			...
			$this->data['meta_title'] =  $this->data['productDetails']->row()->product_name .' Online';
	...
	$PRODUCTNAME = $this->data['productDetails']->row()->product_name;
			...
			if($PRODUCTNAME != '' && $USERNAME != ''){
				$this->data['meta_description'] = 'Buy '. $PRODUCTNAME .' Online in India - by '. $USERNAME .'. Door Step Delivery.';}
