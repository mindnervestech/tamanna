...
class Checkout extends MY_Controller { 
	...
	public function index(){
		...
		if ($this->data['loginCheck'] != ''){
			...
			$this->data['PapiInfo']=$this->PaymentGatewayInfo();//Load Citurs info in Data Veriable
//print_r($this->data['PapiInfo']);
...
/**************************CitrusPay API Integration Method*****************************/
public function PaymentGatewayInfo()
	{
		...
		set_include_path('./authorize/lib_new'.PATH_SEPARATOR.get_include_path());
		require_once('./authorize/lib_new/CitrusPay.php');
		require_once './authorize/lib_new/Zend/Crypt/Hmac.php';
		//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//
		
		CitrusPay::setApiKey($this->config->item('Citrus_SecurityKey'),$this->config->item('Citrus_paymentType'));
        //**CitrusPay::setApiKey('0296c98bc3875be217c32604917971db2c405387','production');
		$vanityUrl = $this->config->item('Citrus_vinityUrl');
		//**$vanityUrl = 'socktail';
		$currency = $this->config->item('Citrus_currency');
		//*$currency = 'INR';
		$merchantTxnId = rand(111111111,9999999999);
		
			$checkAmt = @explode('|',$this->data['checkoutViewResults']);
			if($checkAmt[3] > 0){
		    $orderAmount =$checkAmt[3]; 
			}
		
		$val = "$vanityUrl$orderAmount$merchantTxnId$currency";
		$secSignature = $this->generateHmacKey($val,CitrusPay::getApiKey());
		$url = CitrusPay::getCPBase()."$vanityUrl";  
		$time = time()*1000;
		$time = number_format($time,0,'.','');
		$templateCode = "MTT001";
		$dpFlag ="Yes"; 
		$this->data['ABC']['url']=$url;
		$this->data['ABC']['vanityUrl']=$vanityUrl;
		$this->data['ABC']['currency']=$currency;
		$this->data['ABC']['merchantTxnId']=$merchantTxnId;
		$this->data['ABC']['secSignature']=$secSignature;
		...
		$this->data['ABC']['returnUrl']=base_url().'order/success/'.$loginUserId.'/'.$lastFeatureInsertId;
