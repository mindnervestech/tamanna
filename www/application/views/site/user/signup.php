...
<p style="font-size:12px; text-align:center;line-height:30px;color: #92959c;"> we won't post on your wall without your permission</p>-->
      </div>
   ...
   <!--   <div class="default">
        ...
        <p class="otherway"  <?php if($social_login_session_array['social_login_name'] != ''){?>style="display:none;"<?php }?>><?php if($this->lang->line('signup_signupwith') != '') { echo stripslashes($this->lang->line('signup_signupwith')); } else echo "Sign up with"; ?> <a href="#" onClick="$('.sign .default').hide();$('.sign .email-frm').show();return false;"><?php if($this->lang->line('signup_email_addrs') != '') { echo stripslashes($this->lang->line('signup_email_addrs')); } else echo "your email address"; ?></a> </p>
			...
			<form onSubmit="return register_user();" method="post">
                <fieldset class="frm email-frm" style="display:block;">
                    <?php if($social_login_session_array['social_email_name'] == '') { ?>
                  <!--  <h3 class="stit" style="font-size:15px;font-size: 24px;font-weight: bold;color: #32353c;"><?php if($this->lang->line('signup_with_emailaddrs') != '') { echo stripslashes($this->lang->line('signup_with_emailaddrs')); } else echo "Sign up with your email address"; ?></h3>--><?php } ?>
			...
			$userNewPwd = substr(str_shuffle("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"), 0, $pwdLength);?>
                    ...
                    <span class="tooltip"><i class="icon-info"></i><small><strong><?php if($this->lang->line('signup_for_brandstre') != '') { echo stripslashes($this->lang->line('signup_for_brandstre')); } else echo "For brands or stores"; ?></strong><br />
