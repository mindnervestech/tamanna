	    ...
	    <ol class="cart-order-depth">
	      ...
	      <li class="depth3 current"><span>3</span><?php if($this->lang->line('cart_ord_confirm') != '') { echo stripslashes($this->lang->line('cart_ord_confirm')); } else echo "Order Confirmation"; ?></li>
			...
			$this->output->set_header('refresh:5;url='.base_url().'purchases'); 
            					...
            					<div class="cart-payment-wrap card-payment new-card-payment">
				<strong style="color:#ec1e20;"><?php if($this->lang->line('order_tran_failure') != '') { echo stripslashes($this->lang->line('order_tran_failure')); } else echo "Your payment was not successful. We recommend that you try again with a different payment method."; ?></strong>
                <div class="payment_success"><strong>Reason: <?php echo urldecode($errors); ?></strong></div>
