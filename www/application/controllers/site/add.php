...
class Add extends MY_Controller {
	...
	public function index(){
		...
		if($layoutList->row()->product_control == 'affiliates'){
			...
			$sellingProductDetails = $this->product_model->view_product_details(" where p.status='Publish' and p.quantity < 0 and u.group='Seller' and p.sale_price <1 and u.status='Active' ".$whereCond." or p.status='Publish' and p.quantity < 0 and p.sale_price <1 and p.user_id=0 ".$whereCond." order by p.created desc limit ".$limitPaging);
		}
		...
		$paginationDisplay  = '<a title="'.$newPage.'" class="btn-more" href="'.base_url().$qry_str.'" style="display: none;">See More Ideas</a>';
		...
		if($this->checkLogin('U') != ''){
			...
			if (count($catID)>0){
				...
				$condition = " where p.status='Publish' and u.status='Active' or p.status='Publish' and p.user_id=0" ;
		...
		$this->load->view('site/ideas/add',$this->data);
	}
...
/* Location: ./application/controllers/site/ideas.php */