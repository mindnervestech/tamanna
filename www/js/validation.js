	...
	$.ajax({
		...
		success:function(response){
			if(response ==0){
				...
				$('#Ship_Sub_err').html('Default address doesn`t get deleted.');
	...
	$.ajax({
		...
		success:function(response){
			if(response ==0){
				...
				$('#Ship_err').html('Default address doesn`t get deleted.');
...
//		alert('Maximum stock of this product is '+mqty);
	...
	//alert(product_id+''+sell_id+''+price+''g+product_shipping_cost+''+product_tax_cost+''+attribute_values);
	$.ajax({
		type: 'POST',
...
function update_phone(evt,uid){
	if($(evt).hasClass('updating'))return;
	$(evt).addClass('updating').text('Updating..');
	var phone = $(evt).prev().val();
	if(phone == ''){
...
function update_payment_type(evt,id){
	if($(evt).hasClass('updating'))return;
	$(evt).addClass('updating').text('Updating..');
	var payment_type = $(evt).prev().val();
	if(payment_type == ''){
		alert('Enter valid payment type ');
		$(evt).removeClass('updating').text('Update');
		return false;
	}else{
		$.ajax({
			...
			data:{'payment_type':payment_type,'id':id},
			complete:function(){
				$(evt).removeClass('updating').text('Update');
			}
		});
...
function update_vendor(evt,id){
	if($(evt).hasClass('updating'))return;
	$(evt).addClass('updating').text('Updating..');
	var vendor = $(evt).prev().val();
	if(vendor == ''){
...
function update_sell_id(evt,id){
	if($(evt).hasClass('updating'))return;
	$(evt).addClass('updating').text('Updating..');
	var sell_id = $(evt).prev().val();
	if(sell_id == ''){
		alert('Enter valid vendor ID ');
		$(evt).removeClass('updating').text('Update');
		return false;
	}else{
		$.ajax({
...
function update_courier_name(evt,id){
	if($(evt).hasClass('updating'))return;
	$(evt).addClass('updating').text('Updating..');
	var courier_name = $(evt).prev().val();
	if(courier_name == ''){
		alert('Enter valid courier name ');
		$(evt).removeClass('updating').text('Update');
		return false;
	}else{
		$.ajax({
			...
			data:{'courier_name':courier_name,'id':id},
			complete:function(){
				$(evt).removeClass('updating').text('Update');
			}
		});
...
function update_tracking_id(evt,id){
	if($(evt).hasClass('updating'))return;
	$(evt).addClass('updating').text('Updating..');
	var tracking_id = $(evt).prev().val();
	if(tracking_id == ''){
		alert('Enter valid tracking id ');
		$(evt).removeClass('updating').text('Update');
		return false;
	}else{
		$.ajax({
			...
			data:{'tracking_id':tracking_id,'id':id},
			complete:function(){
				$(evt).removeClass('updating').text('Update');
			}
		});
...
function update_status(evt,id){
	if($(evt).hasClass('updating'))return;
	$(evt).addClass('updating').text('Updating..');
	var status = $(evt).prev().val();
	if(status == ''){
...
function update_shipping_status(evt,id){
	if($(evt).hasClass('updating'))return;
	$(evt).addClass('updating').text('Updating..');
	var shipping_status = $(evt).prev().val();
	if(shipping_status == ''){
		alert('Enter valid shipping_status ');
		$(evt).removeClass('updating').text('Update');
		return false;
	}else{
		$.ajax({
			...
			data:{'shipping_status':shipping_status,'id':id},
			complete:function(){
				$(evt).removeClass('updating').text('Update');
			}
		});
...
function update_received_payment(evt,id){
	if($(evt).hasClass('updating'))return;
	$(evt).addClass('updating').text('Updating..');
	var received_payment = $(evt).prev().val();
	if(received_payment == ''){
		alert('Enter valid received_payment ');
		$(evt).removeClass('updating').text('Update');
		return false;
	}else{
		$.ajax({
			...
			data:{'received_payment':received_payment,'id':id},
			complete:function(){
				$(evt).removeClass('updating').text('Update');
			}
		});
...
function update_exp_dispatch(evt,id){
	if($(evt).hasClass('updating'))return;
	$(evt).addClass('updating').text('Updating..');
	var exp_dispatch = $(evt).prev().val();
	if(exp_dispatch == ''){
		alert('Enter valid exp_dispatch');
		$(evt).removeClass('updating').text('Update');
		return false;
	}else{
		$.ajax({
			...
			data:{'exp_dispatch':exp_dispatch,'id':id},
			complete:function(){
				$(evt).removeClass('updating').text('Update');
			}
		});
